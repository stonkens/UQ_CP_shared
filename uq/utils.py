...
def empirical_copula_loss_new(x, data, epsilon):
    pseudo_data = pseudo_obs(data)
    return np.mean(np.all(np.less_equal(pseudo_data, np.array([x] * pseudo_data.shape[1])), axis=1)) - 1 + epsilon
...
def search_alpha(alpha_input, epsilon, epochs=500, verbose=False):
    ...
    with trange(epochs, desc="training", unit="epochs", disable=not verbose) as pbar:
        for i in pbar:
            optimizer.zero_grad()
            loss = cp(pseudo_data)
